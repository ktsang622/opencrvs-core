...
print_usage_and_exit () {
    echo 'Usage: ./clear-atg-data.sh'
    echo "This script clears all OpenCRVS ATG data locally."
    exit 1
}
...
docker run --rm --network=opencrvs-atg_default mongo:4.4 mongo --host mongo1 --eval "\
...
curl -s "http://localhost:9200/_cat/indices?h=index" | while read -r index; do
  ...
  docker run --rm --network=opencrvs-atg_default appropriate/curl \
    curl -XDELETE "http://elasticsearch:9200/$index" -v
done

docker run --rm --network=opencrvs-atg_default appropriate/curl curl -X POST 'http://influxdb:8086/query?db=ocrvs' --data-urlencode "q=DROP SERIES FROM /.*/" -v
PATH_TO_MINIO_DIR="$DIR/data-atg/minio/ocrvs"
# Clear Minio Data
